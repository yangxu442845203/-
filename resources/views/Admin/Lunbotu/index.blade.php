     ...
     <table class="mws-datatable-fn mws-table dataTable" id="DataTables_Table_1" aria-describedby="DataTables_Table_1_info"> 
      <thead> 
       <tr role="row">
        ...
        <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending" style="width: 211px;">名称</th>
      ...
      @foreach($lunbotu as $row)
       <tr class="odd"> 
        ...
        <td class=" ">
          <form action="/adminlunbotu/{{$row->id}}" method="post">
            {{csrf_field()}}
            {{method_field("DELETE")}}
        <button class="btn btn-success" type="submit"><i class="icon-trash"></i></button>
            <a class="btn btn-info" href="/adminlunbotu/{{$row->id}}/edit"><i class="icon-wrench"></i></a></td>
          </form>
       </tr>
      @endforeach 
