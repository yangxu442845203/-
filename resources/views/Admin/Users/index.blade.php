   ...
   <div class="mws-panel-body no-padding"> 
    <div id="DataTables_Table_1_wrapper" class="dataTables_wrapper" role="grid">
     <form action="/adminusers" method="get">
     	...
     	<label>
        搜索名字: <input type="text" name="keywords"  aria-controls="DataTables_Table_1" value="@if(count($request)){{$k}}
        @else 
        @endif"
        />搜索邮箱: <input type="text" name="keywordss"  aria-controls="DataTables_Table_1" value="
        @if(count($request)){{$k}}
        @else 
        @endif" />
